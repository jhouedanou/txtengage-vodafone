...
onMounted(async () => {
  ...
  // ❌ SUPPRIMÉ : N'initialiser les image-containers que quand slide-23 devient active
  // setTimeout(() => {
...
const initSvgatorAnimations = () => {
  ...
  nextTick(() => {
    ...
    const processSvgObject = (obj, index) => {
      ...
      // ⛔ SKIP : Ignorer les image-containers de slide-23 (gérés par lazy loading)
      if (containerId.startsWith('image-container-')) {
        const slide23Section = document.getElementById('slide-23');
        if (slide23Section) {
          const containerElement = document.getElementById(containerId);
          if (containerElement && slide23Section.contains(containerElement)) {
            console.log(`⏸️ SKIP: ${containerId} appartient à slide-23, sera géré par lazy loading`);
            return;
          }
        }
      }
